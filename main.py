...
def main():
    ...
    display_images = {
        "paths": paths,
        "inputs": tf.map_fn(tf.image.encode_png, convert(unpad_image(rgbxy_to_rgb(de_process(inputs)))), dtype=tf.string, name="inputs_pngs"),
        "targets": tf.map_fn(tf.image.encode_png, convert(unpad_image(rgbxy_to_rgb(de_process(targets)))), dtype=tf.string, name="target_pngs"),
        "outputs": tf.map_fn(tf.image.encode_png, convert(unpad_image(rgbxy_to_rgb(de_process(model.get_outputs())))), dtype=tf.string, name="output_pngs"),
    }
